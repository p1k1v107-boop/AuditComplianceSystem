...
class AuditService:
    ...
    def __init__(self):
        self._repo_audit = JsonRepository(str(Path(DATA_DIR) / "audit.json"))
        self._repo_auditor = JsonRepository(str(Path(DATA_DIR) / "auditor.json"))
        self._repo_unit = JsonRepository(str(Path(DATA_DIR) / "unit_kerja.json"))

    def _pastikan_unit_ada(self, unit_kerja_id: str):
        if not self.detail_unit(unit_kerja_id):
            raise DataTidakValidError("unit_kerja_id", unit_kerja_id, "Unit kerja tidak ditemukan")

    def _pastikan_auditor_ada(self, auditor_id: str):
        if not self.detail_auditor(auditor_id):
            raise DataTidakValidError("auditor_id", auditor_id, "Auditor tidak ditemukan")
    ...
    def tambah_audit(self, data: dict) -> ProgramAudit:
        ...
        self._pastikan_auditor_ada(data["auditor_id"])
        audit = ProgramAudit(
            nama=data["nama"],
    ...
    def perbarui_audit(self, id: str, data: dict) -> ProgramAudit | None:
        ...
        if not audit:
            return None
        unit_kerja_id = data.get("unit_kerja_id", audit.unit_kerja_id)
        auditor_id = data.get("auditor_id", audit.auditor_id)
        self._pastikan_unit_ada(unit_kerja_id)
        self._pastikan_auditor_ada(auditor_id)
        ...
        audit.unit_kerja_id = unit_kerja_id
