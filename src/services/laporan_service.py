...
from src.core.paths import LAPORAN_DIR
...
class LaporanService:
    ...
    def laporan_temuan_terbuka(self) -> str:
        ...
        nama_file = os.path.join(LAPORAN_DIR, f"temuan_terbuka_{self._timestamp()}.txt")
        try:
    ...
    def laporan_keterlambatan(self) -> str:
        ...
        nama_file = os.path.join(LAPORAN_DIR, f"keterlambatan_{self._timestamp()}.txt")
        try:
            with open(nama_file, "w", encoding="utf-8") as f:
                f.write("=" * 60 + "\n")
    ...
    def laporan_unit_berisiko(self) -> str:
        ...
        nama_file = os.path.join(LAPORAN_DIR, f"unit_berisiko_{self._timestamp()}.csv")
        try:
    ...
    def laporan_efektivitas(self) -> str:
        ...
        nama_file = os.path.join(LAPORAN_DIR, f"efektivitas_{self._timestamp()}.csv")
        try:
            with open(nama_file, "w", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow([
