...
def _detail_temuan_context(request: Request, temuan, pesan_error=None, pesan_sukses=None):
    bukti_list = svc.daftar_bukti(temuan.id) if temuan else []
    tl_list = tl_svc.tindak_lanjut_per_temuan(temuan.id) if temuan else []
...
async def detail_temuan(request: Request, id: str):
    ...
    if not temuan:
        return RedirectResponse("/temuan", status_code=303)
    return templates.TemplateResponse("temuan/detail.html", _detail_temuan_context(request, temuan))
...
async def tutup_temuan(request: Request, id: str):
    try:
        ...
        return templates.TemplateResponse(
            "temuan/detail.html",
            _detail_temuan_context(request, temuan, pesan_sukses="Temuan berhasil ditutup."),
        )
    except (BuktiAuditTidakCukupError, DataTidakValidError, StatusTransisiTidakValidError) as e:
        temuan = svc.detail_temuan(id)
        return templates.TemplateResponse(
            "temuan/detail.html",
            _detail_temuan_context(request, temuan, pesan_error=str(e)),
        )
...
async def ubah_status_temuan(id: str, status_baru: str = Form(...)):
    ...
    except (DataTidakValidError, StatusTransisiTidakValidError) as e:
        temuan = svc.detail_temuan(id)
        if not temuan:
            return RedirectResponse("/temuan", status_code=303)
        return templates.TemplateResponse(
...
async def tambah_bukti(
    ...
    if file and file.filename:
        ...
        path_simpan = UPLOADS_DIR / nama_unik
    ...
    try:
        ...
        temuan = svc.detail_temuan(temuan_id)
        if not temuan:
            return RedirectResponse("/temuan", status_code=303)
        return templates.TemplateResponse(
            "temuan/detail.html",
